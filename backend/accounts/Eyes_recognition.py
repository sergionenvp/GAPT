...
def trainer():
    ...
    image_dir = os.path.join(BASE_DIR, "images") #Create path to the images folder
...
def identify():
    eye_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + '/haarcascade_eye.xml') #Take the trained AI from the folder classifier that OpenCV framework provides.
    recognizer = cv2.face.LBPHFaceRecognizer_create() #Create the Local Binary Pattern Histogram
    recognizer.read(os.path.dirname(os.path.abspath(__file__)) + "/trainner.yml") #Read what it has been trained before with the local binary pattern
    labels = {"person_name": 1} #create a dictionary for the labels and their ids
    with open("labels.pickle", 'rb') as f: #Open the file which is saving the labels and id for the images
        og_labels = pickle.load(f) #load the dictionary from the file
        labels = {v: k for k, v in og_labels.items()}#Inverting the id to key and labels to value. Id are uniques
    # Select the webcam of the device.
    cap = cv2.VideoCapture(0)
    person_detected_percentage = {} #Dictionary to save the recognized people by the program 
    ...
    while cap.isOpened():  # While the camera is opened
        # Capture the image
        ret, frame = cap.read()  # Save the frame (image)
    ...
    #--------------------------------------------
    return Response("It recognized " + str(possible_people)  +  " with a probability grater than 20%", status=200)
